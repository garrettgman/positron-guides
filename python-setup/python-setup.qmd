...
Environments act as a partner to your code. The same Python code may return a different result—or not run at all—when it is run in a different environment. This creates an obvious problem for reproducibility, but the problem has a solution: be intentional about the Python environments you use, and document those environments alongside your code.

The Positron IDE can help you do both. In this guide, you'll learn how to use Positron to manage **virtual Environments**, which are Python's [built-in environments](https://docs.python.org/3/glossary.html#term-virtual-environment). A virtual environment is a subdirectory that contains a copy or a symlink to a Python interpreter, as well as a library of packages to be used by the environment.
...
If you want more control than the drop down menu can provide, consider using the [Python: Select Interpreter](https://code.visualstudio.com/docs/python/environments#_working-with-python-interpreters) command from the Command Palette. Python: Select Interpreter is one of the many commands that Positron inherits from its upstream parent, Visual Studio Code, which Positron is a fork of. To access the Command Palette, press `⇧⌘P` on a Mac and `Ctrl + Shift + P` on Windows/Linux. Then start typing the name of the command.
...
3. Then select the type of environment you would like to create. There are multiple environment systems for Python, and Positron supports the most popular. For this guide, we will choose Venv.

![Select type](images/type.png)

4. Next select the Python interpreter you would like to use for the environment.

![Select interpreter](images/interpreter.png)

Positron will create the environment in you current folder and then activate it for use. Along the way, Positron will do several things:
...
- If you haven't installed the ipykernal package, Positron will ask you to install it.
...
Let's look at one popular way to do that.

### requirements.txt

You can use the `pip freeze` command at the terminal to record the packages that your project depends on, along with their versions:
...
Alternatively, they can easily install the pacakges manually from the command line with.

```bash
pip install -r requirements.txt
```

Often, `requirements.txt` will be enough to make your package reproducible, but you can also specify the Python version your project uses, as well as additional metadata, by creating a `pyproject.toml` file with [poetry](https://python-poetry.org/) or [uv](https://docs.astral.sh/uv/).
